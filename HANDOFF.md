...
| 2026-05-09 | **Reverted the email-confirmation flow** (commits `a403518` magic-link callback + `f039260` OTP code-entry signup) in commit `ee93f1a` | The Supabase dashboard `Confirm email` toggle wasn't sticking during testing — signups kept auto-confirming despite the setting. Rolled back the route handler at `/auth/confirm`, the `signUp` `emailRedirectTo`, the 2-step OTP signup form, and the `verifySignup` / `resendSignupOtp` actions. Will revisit with a cleaner approach (likely fixing the dashboard toggle first or wiring custom SMTP). Player-access hardening (`3cad90a`) and the searchable email dropdown (`dc4766b`) stayed intact through the revert. |
