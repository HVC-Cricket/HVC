...
**2026-05-19 (batch 43) — App icon: round HVC artwork + dev/prod split.** Pavan flagged that "Add to Home Screen" was producing a plain `HVC` text tile generated by the four icon route handlers (`icon.tsx`, `icon1.tsx`, `icon2.tsx`, `apple-icon.tsx`) and asked for actual artwork — and, separately, a way to tell the dev install apart from the prod install on the same phone.
