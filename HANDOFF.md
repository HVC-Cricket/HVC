...
Auto-hides once every group match flips to `completed` (the organizer should be scheduling real playoff matches by then). Also a no-op for any other tournament format. Inline `PlayoffPlaceholders` + `TbcRow` helpers at the bottom of the file. 1 file / +85 LOC.
...
Added `timeZone: "Asia/Kolkata"` to every user-visible date / time renderer:

- `src/lib/format.ts` — shared `IST` constant; applied to `formatScheduledAt`, `formatUpcomingTime`, `formatMatchTime`, and `formatDateRange`. The range formatter's same-day / same-month checks were rebuilt around `toLocaleDateString("en-CA", {timeZone: IST, ...})` equality keys instead of `toDateString()` / `getMonth()` (both of which read in the runtime locale and could break across IST midnight).
- `src/app/matches/[matchId]/opengraph-image.tsx` — scheduled-fallback string in the OG image.
- `src/app/tournaments/[slug]/opengraph-image.tsx` — tournament date range in the OG image.
- `src/app/matches/[matchId]/activity/page.tsx` — per-event timestamps in the activity log.
- `src/app/home-past-tournaments.tsx` — past-tournament date range; year-equality check switched from `getUTCFullYear()` to an IST formatter so a 23:00-IST timestamp doesn't fall on the wrong year.

`audit-log-section.tsx` and `members-table.tsx` (admin-only) deliberately left alone — same fix needed but lower priority since admin views aren't tied to wall-clock scheduling semantics.

Same session: dropped `scripts/seed-hvc-season-7-schedule.sql` for the live HVC Season 7 tournament. 21 round-robin matches across 23-May-26 and 24-May-26, source = `HVC_Schedule_Final.xlsx` (sheet `"Sheet9 (2)"`). Mirrors the existing `seed-appis-tournament.sql` pattern — looks up `tournaments.name = 'HVC - SEASON 7'` and the seven team rows by case-insensitive name match, inserts one matches row per slot with HVC defaults (`overs_per_innings=7`, `players_per_side=7`). One-shot only — `matches` has no unique `(tournament_id, match_number)`, so re-runs duplicate. Trailing `count(*)` sanity check has the lookup inlined (CTEs only scope to one statement; the earlier version errored on `relation "tournament" does not exist`).

Commits `5846c05` (schedule) + `08b8997` (IST formatters); 6 files / +166 / −9 LOC.

**2026-05-19 (batch 45) — App icon: round HVC artwork + dev/prod split.** Pavan flagged that "Add to Home Screen" was producing a plain `HVC` text tile generated by the four icon route handlers (`icon.tsx`, `icon1.tsx`, `icon2.tsx`, `apple-icon.tsx`) and asked for actual artwork — and, separately, a way to tell the dev install apart from the prod install on the same phone.

Replaced the hard-coded text rendering with an environment-aware system driven by a new `NEXT_PUBLIC_APP_ENV` env var:

- **prod (or unset)** — renders `public/app-icon.png` (the HVC batter artwork the user supplied) clipped to a circle. Each route handler hands the artwork to an `<img>` inside `ImageResponse` with `objectFit: "cover"` + `borderRadius: "50%"`. The file is loaded with `fs.readFileSync` **once at module init** and inlined as a base64 data URL — that side-steps the fragility of hard-coding a deployment URL inside the route, and the data-URL approach is what Satori (the renderer behind `next/og`) actually accepts.
- **dev** — keeps the dynamic text treatment but on an amber palette with `DEV` (32×32) or `HVC DEV` (180+) labels. Visually unmistakable when both PWAs sit side-by-side on a home screen.

New shared module `src/app/icon-theme.ts` owns `ICON_PROD` / `ICON_DEV` theme constants, the `isDevIcon()` env check (`process.env.NEXT_PUBLIC_APP_ENV === "dev"`), and the lazily-loaded `PROD_ARTWORK_DATA_URL` (null when the artwork file is missing — handlers fall through to the text treatment so the build doesn't crash). All four route handlers consume it.

`.env.local.example` got a documented `NEXT_PUBLIC_APP_ENV` line so the var is discoverable on a fresh checkout. Default behaviour (env var absent) is the prod theme — a production deploy that forgets the var still gets the right look.

Caveats for the rollout: the home-screen icon is OS-cached aggressively, so the PWA needs to be removed and re-added to pick up the new artwork. `icon1` / `icon2` are still declared `purpose: "maskable"` in the manifest, so Android will further mask the rounded artwork into whatever shape its launcher prefers — the batter + HVC text sit in the inner safe zone so that shouldn't crop important content, but worth keeping in mind if Android renderings look off.

Commit `9417f72`; 7 files / +199 / −17 LOC (incl. the 2.5 MB artwork PNG).
...
- **#7 Server-side per-ball cat enforcement.** `recordBall` now loads `matches.rules_override`, computes `effectiveRules = applyRulesOverride(rules, override)`, and for the current `state.current_over_number` checks `categoryForOver(effectiveRules, n)`. If the over is Cat 1 / Cat 3 (i.e. not Cat 2 open), striker AND bowler must match — both checked against `playerById` from the existing fetch. Mirrors the client-side `overCategory !== 2` toast in scoreboard.tsx so a tampered client can't slip a Cat-2 ball into a Cat-N over.
- **#6 Player-edit warning on live-match XIs.** `/players/[id]/edit` now queries `match_players` joined to `matches` with `status in ('live', 'innings_break')`, filtered to the player + `is_substitute = false`. If non-empty, an amber card surfaces above the form: "Changing category mid-match may break Cat-N over rules" + links to each affected match. Read-only — doesn't block the save; just informs.
- **#8 Unit tests for the new helpers.** `src/lib/scoring/__tests__/rules.test.ts` covers `categoryForOver`, `applyRulesOverride`, `findCategoryGaps`. 16 new tests (37 total). Round-trip-tested edge cases: legacy scalar shapes, multi-over arrays, empty arrays, override pass-through, mutation safety.
- **#9 Tournament-create form** now exposes `cat1_overs` / `cat3_overs` via the same `CategoryOversFields` component. Seeded with the HVC default (`[1]` / `[2]`); organisers can clear or rearrange before hitting Create. `createTournament` action merges them into the HVC ruleset on insert (rest of `RuleSet` — super_over, extras, etc. — passes through unchanged).
- **#10 "Clear both" button** on `CategoryOversFields`. One click resets both arrays to `[]` so a non-HVC tournament or "Cat 2 only" match doesn't require untiking every chip individually. Hidden when both rows are already empty.

11 files / +250 / −30 LOC + 1 new test file.
...
**2026-05-18 (batch 30) — `/players` category filter chips.** Added a small chip row above the search input: **All / Cat 1 / Cat 2 / Cat 3 / No cat** with per-bucket counts (computed off the unfiltered list so each chip always shows its total). Selecting a chip toggles it on/off; reclicking the active one clears. Active state uses the same colour accents as the rest of the app — amber for Cat 1, sky for Cat 3, destructive for "No cat", default for All / Cat 2. Filter combines with the existing name search — typing "Aditya" + selecting Cat 2 narrows to Cat-2 Adityas. "No cat" chip hides when there are zero uncategorised players so the row stays clean.
...
2. **Delete user on `/admins`.** Each member row gets a small trash button next to Promote/Demote, except on the row that belongs to the signed-in super-admin (no self-delete from here). `deleteUser` action gates on `requireSuperAdmin` server-side AND refuses to delete the last super-admin so the org can't lock itself out. Calls `auth.admin.deleteUser` via the admin client; the existing cascades / set-nulls handle the rest (`profiles` cascade, `players.linked_user_id` → null, `tournament_admins` cascade). Match history is untouched because `balls` / `innings` / `match_players` reference `players`, not `auth.users`.

3. **Mobile popover scroll-up fix.** Pavan flagged that tapping the **Link player** combobox on `/admins` on mobile scrolled the whole page up and hid the dropdown. Root cause: the popover used `align="end"` on a trigger that's right-aligned on desktop but left-aligned on mobile (the row is `flex-col items-start sm:items-end`); base-ui's collision detection was flipping the popup to a position offscreen above the trigger, and the auto-focused `CommandInput` then made the browser scroll up to bring the input into view. Fixes: `align="start"` (popup grows to the right of the trigger, which fits both layouts), responsive width `w-[min(280px,calc(100vw-1.5rem))]` so the popup can never overflow the viewport horizontally, and `max-h-[60vh]` so it can never get tall enough to need a flip. Same width fix applied to the `AddSquadMemberPopover` for consistency.

4 files / +160 / −18 LOC.

**2026-05-18 (batch 26) — `XISection` lists the whole squad, not just match_players.** Pavan flagged: added a player to a team via the new inline "Add player" popover, confirmed on the team page, then the match page still showed only the previous 5 names. Root cause: `XISection.TeamXICard` queried `match_players` only — squad members without a `match_players` row (i.e. anyone added to the squad but not yet picked into the XI via Pick XI) were invisible on the match page.

Fix: query **both** `match_players` and `team_players` in parallel, then build a unioned row list — every squad member shows up, with `match_players` info overlaid where available (`is_substitute`, `batting_order`, `is_captain`, `is_keeper`). Squad members without a `match_players` row are treated as implicit subs for the playing-count math but rendered with a small "**not picked**" badge instead of the "sub" suffix, so it's obvious which subs are deliberate vs which are just-added-to-squad-and-need-Pick-XI. Sort order: in-XI by `batting_order` first → in-match subs → not-yet-picked squad members.

Edge case for completeness: if a `match_players` row exists for a player who's been since removed from the squad (rare), we still include them — keeps the chip / breakdown honest.
...
- **`/matches/[id]/xi/[teamId]` (Pick XI page)** now renders an amber banner above the form when the roster is below `players_per_side`, with the same copy + a "Manage squad →" link straight to the team page (`/tournaments/[slug]/teams/[teamId]`). The existing "No squad" empty state got the same link. Page now fetches `tournaments(slug)` in parallel with the team row.

Diagnosis: the user had added 5 players to the team squad on the team page, then visited Pick XI and marked all 5 as subs (or just opened it without confirming). The `match_players` rows showed 5 × `is_substitute=true`, so `playing.length = 0` and the description's `playersPerSide - playing.length = 6` was technically correct but useless guidance. The new copy surfaces the root cause (squad shortage) and the action (Manage squad).
...
2. **Phone inputs accepted letters everywhere.** `/me`, `/players/new`, `/players/[id]/edit` all rendered `<Input type="tel" />` — `type=tel` is just a hint to mobile keyboards; it doesn't restrict character set. New `src/lib/phone.ts` exports `PHONE_ALLOWED_RE = /^[0-9+\-\s()]*$/` and `stripPhoneInput()`. All three forms wire `onChange={(e) => field.onChange(stripPhoneInput(e.target.value))}` so typing a letter just yields no character. Zod schemas on those three forms + the server-side schemas in `me/actions.ts` and `players/actions.ts` (`phoneField` shared between create/update) now `regex(PHONE_ALLOWED_RE)` so a tampered client can't bypass. Format is permissive: digits, leading `+`, spaces, dashes, parens — no country-code enforcement, just visually-numeric.

6 files / +63 / −10 LOC.

**2026-05-18 (batch 16) — /me edit form exposes player fields; phone is self-service.** The `/me` Edit pane only let the user change `display_name` + `avatar_url`. Anything player-shaped — `category`, `phone`, `batting_style`, `bowling_style` — required a trip to `/players/[id]/edit`. Now the `/me` edit form renders a "Player profile" block whenever the user has a linked player row, with two trust tiers:

- **Phone** — editable by any linked player (it's their own contact info). Always shown inside the player block.
- **Category / Batting / Bowling** — super-admin only. Gated client-side (block hidden) AND server-side (`is_super_admin` re-checked inside `updateProfile` before touching those columns).

Submit goes through a single `updateProfile` action: profile row always updates `display_name` + `avatar_url`; the player-row update only fires when at least one player-field column would actually change, so a non-super-admin who didn't change phone doesn't pointlessly hit the DB. `display_name` ↔ `players.display_name` and `avatar_url` ↔ `players.photo_url` still flow through the existing sync triggers — those weren't touched.

Non-linked users (signed in but no `players.linked_user_id` row) still see only display-name + avatar.

Props: `EditProfileForm` takes `player: PlayerFieldsInitial | null` + `canEditAdminFields: boolean`. `ProfileCard` forwards `editablePlayerFields` + `canEditAdminPlayerFields`. `me/page.tsx` passes `editablePlayerFields` whenever `linkedPlayer` exists and `canEditAdminPlayerFields={isSuperAdmin}`.

4 files changed across two passes (initial super-admin-only build + phone-for-everyone follow-up).

**2026-05-18 (batch 15) — "Add to squad" player picker becomes a searchable combobox.** The player Select on `/tournaments/[slug]/teams/[teamId]` was the shadcn (Radix) Select — keyboard-driven with no visible search input. With 60+ players in the dropdown the only filter was Radix's type-ahead jump, which doesn't show what was typed. Replaced with a combobox built on shadcn's `Popover` + `Command` (cmdk). Now: trigger renders the selected name + chevron; clicking opens a popover with a real `CommandInput` (typed text stays visible), filtered `CommandList`, "No player found." empty state, and a check icon next to the active row.

Two gotchas worked through:
- **Duplicate display_names** (e.g. two "Aditya"s): cmdk de-dupes items by `value`. Suffixing `value` with the player id (`${display_name} ${id}`) keeps each row distinct. A custom `filter` strips the trailing id token before matching so multi-word names like "Anantha Hari" stay searchable.
- **`asChild` doesn't exist on this shadcn build.** The popover component on this repo is `@base-ui/react/popover`, not Radix — its trigger uses `render={(props) => <element {...props} />}` for composition. Switched from `<PopoverTrigger asChild><FormControl><Button…/></FormControl></PopoverTrigger>` to `<PopoverTrigger render={(props) => <Button {...props}…>…</Button>} />`. Dropped FormControl on this field — its only contribution was `aria-describedby` wiring to FormMessage, which doesn't apply to a combobox trigger; the visible FormMessage still renders on validation failure.

shadcn `add popover command` pulled in 5 ui files (popover, command, plus transitive textarea/dialog/input-group). Only the two are used today.

**Don't propagate this pattern blindly to every other player Select** — the Pick XI form is checkbox-based, the wicket-modal picker has its own filters (already-out / off-category / last-man), the bowler picker filters by category + consecutive-over rule, etc. The searchable combobox is a fit when the choice is a single player out of an unfiltered roster of ~60+ rows. Reach for shadcn `Command` again if/when another such surface shows up.

3 files (1 form + 5 new ui from shadcn) / +66 / −11 LOC.
...
Separately, the commentary feed's header strip ("Commentary" + "Latest balls at the top. Auto-generated from each delivery.") was removed — it became noise once the feed moved out of its own tab (batch 9) and into the Live panel below the score, where the previous card's heading is enough context. Dropped the `CardHeader` block + the now-unused `CardHeader / CardTitle / CardDescription` imports.

Commits `bdd0e81` (Squads tab) + `4ef64dc` (commentary header); 3 files / +24 / −28 LOC.
...
Fix: compute `localIsSpecialOver` at submit time from `playersById.get(strikerId)?.category` — mirrors the engine's `computeSpecialOverContext`, which keys off the striker's category. Pass it through `applyOptimisticRotation`'s `active.is_special_over` field (replacing the lagging server value) and use it in the `isSpecialStay` gate that suppresses the wicket-replacement dialog. The category-mismatch validation toast in `submit` was using the same `playersById.get(strikerId)?.category` lookup, so it was hoisted to share the `localStrikerCat` binding.

Same session: disable the **Swap button** when `overCategory !== 2`. Under the stay rule the Cat 1 / Cat 3 striker must remain on strike for the full over, so a manual mid-over swap breaks the engine contract (and contradicts the optimistic-rotation's odd-run swap suppression). The button's existing `last_man_mode` gate stays, with a new branch in the tooltip that reads "Disabled — Cat X striker must stay on strike".

Commit `153318e`; 1 file / +29 / −5 LOC.

**2026-05-18 (batch 5) — Scoreboard: wicket prompt + skip-after-over-1 + Cat stay preservation.** Three follow-ups to batch 4 from continued live-scoring feedback.

First, the wicket flow got the same instant-prompt treatment as the over-complete flow. The moment a wicket is recorded, an `AlertDialog` opens with the batting XI in a `Select` — disabled rows for already-out batters, the live batter at the other end, and (when the new batter is going to strike for a Cat 1 / Cat 3 over) off-category candidates. OK is disabled until the relevant slot has a value; the pick writes straight to `strikerId` / `nonStrikerId`. Suppressed when no replacement is actually needed: innings ending on this wicket (`isComplete` flipping), last-man rule kicking in (or already on), and Cat 1 / Cat 3 special-batter "stay" dismissals where the engine keeps the dismissed batter at the crease for the remaining balls of the over. The wicket dialog and the over-complete dialog are sequenced — the over-complete `open` prop checks `wicketPrompt === null`, so a 6th-ball wicket surfaces the batter prompt first and the bowler prompt only after dismissal.

Second, the over-complete prompt is no longer shown after over 1. The over 1 → over 2 boundary is Cat 1 → Cat 3, and the auto-pick effect on category change already fills the bowler slot with a Cat 3 candidate — the dialog was just asking the scorer to confirm a foregone auto-pick. From over 2 onwards the prompt earns its keep (Cat 2 has no auto-pick guardrail; super overs the same). Gate applied to both the optimistic submit-time trigger and the server-fallback `over_number` effect so a page reload between overs 1 and 2 doesn't surface it either.

Third, Pavan flagged that "in the 1st over, if selected category is Cat 1 and the Cat 1 player is out, the striker dropdown should not get cleared out because he will bat till the end of the over" (and same for Cat 3 in over 2). Root cause: `applyOptimisticRotation` was wiping the dismissed slot unconditionally, so even though the *engine* keeps the special batter at the crease under the `cat_special_strike: "stay"` rule, the *client* flashed the slot empty for the 1–3 s recordBall roundtrip. Comment at the top of optimistic-rotation.ts acknowledged this as a known mis-prediction ("rare edge cases like the dismissed-special-batter race"). Fixed by adding `special_stay_rule: boolean` to `ActiveState` (mirrors `stillSpecialStay` in state.ts) and skipping the wicket clear when `is_special_over !== null && special_stay_rule && dismissed === striker`. Scoreboard passes `state.rules.categories.cat_special_strike === "stay"`. Non-special-batter dismissals in a special over (e.g., non-striker run-out) still clear the slot — that's the correct engine behavior.

Pick-preservation logic extended from the bowler slot (batch 4) to the batter slots: the balls-length sync now reads `state.active.* ?? current` for striker / non-striker, so an optimistic wicket-dialog pick on a mid-over or end-of-over dismissal survives the recordBall roundtrip instead of being clobbered when the server's reconciliation lands. The Cat 1 / Cat 3 boundary path keeps its no-op-when-already-empty form via `(current) => current || ""`.

Commit `06c5cec`; 2 files / +208 / −16 LOC.
...
2. **Rank badges on `/players/[id]` + `/me`.** New "All-time ranks" card between Career and By-tournament. Surfaces every metric where the player ranks ≤ 10 across all HVC seasons (runs / wickets / fours / sixes / matches / POM). Dense rank — ties share a number; the next distinct value gets the next rank. Visual hierarchy: rank 1 = primary-coloured pill, 2–3 = foreground, 4–10 = muted. Source: new `src/lib/stats/career-ranks.ts` fetches `v_player_tournament_stats` + paginated `match_players` + matches with `player_of_match_id`. Ranks computed server-side; only badge-eligible entries (rank ≤ 10) returned.

3. **Historical fielding via dismissal-text parsing.** Cricheroes doesn't expose per-ball fielder IDs, but the `how_to_out` text in `historical_match_batting` does ("c Pavan Kashyap b Mahesh", "run out Tej", "c&b Aprameya Guruprasad", "st Bharath b Sudharshan"). New `src/lib/stats/historical-fielders.ts` regexes out four dismissal types (CATCH_RE, C_AND_B_RE, STUMPED_RE, RUN_OUT_RE) and matches the captured name case-insensitive-exact against `players.display_name`. `buildPlayerNameLookup()` normalises (lowercase, strip `†` dagger, collapse whitespace) and marks duplicates as `"ambiguous"` — those silently skip rather than risk a wrong credit. `tournament-stats.tsx loadHistoricalStats` and `career-stats.tsx` both run the parser over already-fetched `historical_match_batting` rows and merge credits into the existing `FieldAgg` pipeline. Player fetch widened (`.limit(5000)` instead of `.in("id", playerIds)`) so the parser resolves against the whole roster. Verified on prod: 183 catches + 20 c&b + 11 stumpings + 139 run-outs in the table → ~360 fielder credits populate FIELD on S1–S6 once deployed.

4. **Vercel deploy unblock.** Coworker's `scripts/merge_players.ts` (commit `c21bcfd`) had a Supabase `.update()` generics mismatch that broke `next build`'s typecheck. Through that, every Vercel deploy of every subsequent commit failed. Scripts run via tsx, not as part of the app bundle. Fix: `"exclude": ["node_modules", "scripts"]` in `tsconfig.json`. Deploys resumed.

Commits `b855b1f` (rank badges + AWARDS+OTHER, initial as MISC), `edffc96` (tsconfig unblock), `6df8ccb` (historical fielders + AWARDS/OTHER rename).

**2026-05-19 (batch 1) — All-time stats page at `/stats` — HVC Heroes records.** Pavan asked: "can we have total stats like most runs, most wickets, etc from total all seasons?" — yes. Surfaced behind a new nav drawer link **HVC Heroes** (page H1 reads the same) at `/stats`. Career leaderboards across every match in every tournament; data merges live-scored balls (S7+) with cricheroes-imported historical aggregates (S1–S6) by `player_id`. Same BAT / BOWL / FIELD UI (pill strip, style dropdown across 17 leaderboards, Cat 1/2/3 chip filter, pagination) as the per-tournament Stats tab — `tournament-stats-view.tsx` is reused unchanged.
...
- [x] **Deterministic standings tiebreaker (2026-05-17 late)** — `computeStandings` previously relied on JS `Array.sort`'s stability and the unspecified row order from `v_points_table` (no `ORDER BY` on the view) when teams were tied on points + NRR. That was fine while NRR was unique per team, but the pre-FK-disambiguation bug returned NRR = 0 for everyone, leaving the playoff auto-scheduler picking `standings[2..3]` from whatever order Postgres happened to return — which in one tournament put a top-2 team into the Eliminator slot. Added `a.team_id.localeCompare(b.team_id)` as the third sort key so the order is fully deterministic regardless of Postgres's row return order. Doesn't affect the rendered standings page (already had a name-based tiebreaker on top), only the auto-scheduler's positional lookup. Also refreshed the `maybeAutoSchedulePlayoffs` docstring to describe the current parallel-fire behaviour.
- [x] **Over-boundary slot policy: clear striker + bowler, conditional non-striker (2026-05-17 late)** — at every over boundary the scoreboard now clears slot picks per HVC's category convention:
   - **Cat 1 / Cat 3 overs (1, 2)**: clear striker + bowler, **keep non-striker** (whoever was on strike last over is now at non-strike post-rotation; they continue). Striker + bowler auto-fill with the first eligible Cat-matching candidate via a `useEffect` keyed on `overCategory` — only acts on empty / off-category slots and never touches non-striker.
   - **Cat 2 overs (3+) and super-over boundaries**: clear **all three** slots — Cat 2 is "any category", no rule guiding the auto-pick, so the scorer chooses both ends fresh.
   - The over-boundary clear lives inside the existing `balls.length` sync effect (`src/app/matches/[matchId]/score/scoreboard.tsx`); the new gate is `defaultOverCategory(state.active.over_number) === 2 || isSuperOver` → clear non-striker too, else preserve it. Initial-mount + innings-2-start unaffected (`balls.length === 0` short-circuits the boundary branch); innings-start picks from `StartMatchPanel` / `InningsBreakPanel` still seed the slots.
