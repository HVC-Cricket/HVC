...
**2026-05-18 (batch 16) — /me edit form exposes player fields for super-admins.** The `/me` Edit pane only let the user change `display_name` + `avatar_url` (written to `profiles`). Anything player-shaped — `category`, `phone`, `batting_style`, `bowling_style` — required a trip to `/players/[id]/edit`, which super-admins linked to their own player could reach but it was a bounce. Pavan flagged that as a super-admin he should be able to manage everything from `/me`.

Now `/me` renders an additional "Player profile" block inside the edit form when **(a)** the viewer is a super-admin and **(b)** they have a linked player row. The block contains Category / Phone / Batting / Bowling pickers identical to the `/players/[id]/edit` set — submit goes through a single `updateProfile` action which still always updates `profiles.display_name` + `profiles.avatar_url` first, and then conditionally updates `players.{category,phone,batting_style,bowling_style}` after re-verifying the super-admin + linked-player gate server-side (the client gate only hides the UI). `display_name` ↔ `players.display_name` and `avatar_url` ↔ `players.photo_url` still flow through the existing sync triggers — those weren't touched.

Scope is **super-admin only** by design — regular organizers editing their own player record still go via `/players/[id]/edit` like they always did. The narrower gate avoids opening "edit my own category" to anyone who's an organizer of any tournament; loosen later if/when there's a reason.

4 files / +276 / −5 LOC. `me/actions.ts` schema gained the four optional player fields, `edit-profile-form.tsx` gained the conditional block, `profile-card.tsx` forwards a new `editablePlayerFields` prop, `me/page.tsx` fetches `phone` alongside the existing player fields and computes the prop.
