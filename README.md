...
- 🚧 **Phase 6** — Polish.
  ...
  - ✅ **Player photos on leaderboards + team squad rows (2026-05-20).** Avatars surfaced on `/stats` leaderboards (all-time + per-tournament) and the team squad page at `/tournaments/[slug]/teams/[teamId]`. Resolution: `players.photo_url` → linked user's `profiles.avatar_url` → initials fallback (same chain used everywhere else — player list, XI cards, profile header). Leaderboards: 28px avatar between rank circle and player name; player column widened (140 → 170px mobile, 200 → 220px sm+) and name `line-clamp-2` so HVC's longer names ("Pradhdhyumna Kashyap HP (Wk)") stop wrapping into 4 lines and misaligning rows. Squad rows: 36px avatar before each player. Currently ~7% photo coverage on prod with ~12% linked accounts; rest render initials and fill in as sign-ups + linking happen during the tournament. Leaderboard rows now also Link into `/players/[id]` so the rank badges card is discoverable from there.
  - ✅ **Historical fielder accuracy: 84.5% → 100% (2026-05-19).** Three rounds of refinement on the dismissal-text parser. (a) **Per-match roster lookup.** First version matched fielder names against `players.display_name` globally — but renames and merges in our app meant the cricheroes-original name no longer matched anyone (e.g., "Bharath G S Agasthya" → renamed to "Bharath Foundry"; multiple "Amith" cricheroes IDs → merged to one "Amith P" UUID). Switched to looking up against `historical_match_batting.player_name` + `historical_match_bowling.player_name` scoped to the same match — those columns preserve cricheroes-original naming AND the `player_id` already points at the current merged UUID. Bridges renames + merges automatically with no schema change. Took coverage from 84.5% → 98.9%. (b) **Strip cricheroes parenthetical suffixes.** Rosters carry "Yashu  (wk)" and "Badri (5a)" but the dismissal text drops the suffix. `normalize()` now strips any trailing `(…)` group on both sides. 98.9% → 99.8%. (c) **First-name token fallback.** For subs / fielders absent from the match's batting + bowling rosters, fall back to a global first-name match — only credits when exactly one player's name starts with that token (ambiguous tokens skip). After Pavan's merge work, "Amith" uniquely points at "Amith P". 99.8% → **100% (626/626)**. New tournaments (S7+) sidestep all this — they use `balls.fielder_id` directly, no parsing involved.
  - ✅ **HVC Heroes follow-ups: AWARDS / OTHER split, profile rank badges, historical fielding (2026-05-19).** Polish on the all-time stats page. (a) MISC pill split into **AWARDS** (Most Player-of-the-Match) + **OTHER** (Most Matches Played) — "MISC" read as cryptic. Each tab hides independently when its data is absent. (b) **"All-time ranks"** card on `/players/[id]` + `/me` — shows badges where the player ranks ≤ 10 across runs / wickets / fours / sixes / matches / POM. Dense rank (ties share a number); rank 1 gets a primary-coloured pill, 2–3 foreground, 4–10 muted. Source: new `src/lib/stats/career-ranks.ts` pulls aggregated `v_player_tournament_stats` + paginated `match_players` + matches.player_of_match_id, ranks server-side, returns only badge-eligible entries. (c) **Historical fielding via dismissal-text parsing.** Cricheroes commentary doesn't expose per-ball fielder IDs but the `how_to_out` text does ("c Pavan Kashyap b Mahesh", "run out Tej", "st Bharath b Sudharshan"). New `src/lib/stats/historical-fielders.ts` regexes out catches / c&b / stumpings / run-outs and matches names case-insensitive-exact against `players.display_name`. Ambiguous or unmatched names silently skip (best-effort). On prod that lights up ~360 historical fielder credits across S1–S6 — the FIELD tab now populates on /stats AND on per-tournament Stats for S1–S6 (was empty before). (d) Fix: `tsconfig.json` excludes `scripts/` from typecheck — a coworker's `merge_players.ts` had a Supabase generics mismatch on `.update()` that was breaking every Vercel deploy. Scripts run via tsx and don't need to gate the build.
  - ✅ **All-time stats page at `/stats` — HVC Heroes records (2026-05-19).** New top-level page showing career leaderboards across every match in every tournament — live-scored balls (S7+) merged with cricheroes-imported historical aggregates (S1–S6) by `player_id`. Same BAT / BOWL / FIELD pills, style dropdown (Top Runs · Highest Scores · Most 4s / 6s / 50s / Best SR / Avg · Most Wickets · Best BBI / Avg / Econ / SR · Most Maidens / Dots · Most Catches / Run-outs / Stumpings), Cat 1 / 2 / 3 chip filter, and pagination as the per-tournament Stats tab — just no tournament filter. Refactored the aggregation core out of `tournament-stats.tsx` into `src/lib/stats/aggregate.ts` so both paths share `newBatAgg` / `newBowlAgg` / `newFieldAgg` / `accumulate*` / `buildLeaderboards` / minimum-sample constants (`MIN_BAT_BALLS`, etc.). Career path paginates large fetches in 1000-row pages via `.range()` (the project's `db.max_rows = 1000` was silently truncating the 1602-row `historical_match_batting` table and dropping ~228 of Teju's career runs); widens the team lookup to include every innings' both teams so the "vs" column on Highest Scores doesn't render `?` when a historical bowling XI has unlinked CricHeroes names. Nav drawer link reads **HVC Heroes**; page H1 reads **HVC Heroes**.
  - ✅ **Pick XI: combined two-team picker with tabs (2026-05-18).** The setup flow was four nav events for one task (score → tap Team A → save → back → tap Team B → save → back), and scorers were starting matches with only one XI picked because the score page didn't strongly signal Team B was still empty. New route `/matches/[matchId]/xi` carries both teams as tabs with progress badges; save buttons relabel to "Save & next team" / "Save & done" based on state. The XISection on the score page swaps two per-team Pick XI buttons for one combined "Pick playing XIs" CTA, funnelling everyone through the tabs flow. `PickXIForm` gained optional `saveLabel` + `onSaveSuccess` props so it's reused inside the tabs without duplicating the table / state. The per-team route (`/xi/[teamId]`) still works as a deep-link escape hatch. Follow-ups same day: underline-style tabs matching MatchTabs, sticky full-width Save bar, "Save & done" label fires when the OTHER team is already at the target count (not just when explicitly saved this session), and post-save navigation switched from `router.back()` to explicit `router.push(/matches/{id})` so users reaching the picker via a direct URL / redirect chain don't get bounced to home.
  - ✅ **Player ↔ profile sync now triggers on the link event (2026-05-18).** The existing two-way sync triggers (avatar + display_name from 2026-05-16) only fired on UPDATE OF those columns — they missed the case where a user signed up, updated their profile name, AND THEN got linked to a player. Bharath Foundry hit exactly this on prod: profile said "BHARATH G S AGASTHYA", player row stuck at "Bharath Foundry". New migration `20260518000000_sync_player_on_link.sql` adds BEFORE-triggers on `players.linked_user_id` UPDATE + INSERT that pull `display_name` + `photo_url` from the linked profile in the same statement (no second UPDATE, no recursion). Includes a backfill UPDATE that fixed every already-linked-but-mismatched player. Audit post-apply: 0 mismatches across 5 linked players on prod.
  - ✅ **Scoring lock: auto-expire pending requests after 120 s (2026-05-18).** A takeover request that sat unanswered used to stick around forever — the requester was stuck on "Waiting for permission" and the holder kept seeing the same banner across reloads. New `PENDING_REQUEST_EXPIRY_SECONDS = 120` (symmetric with `LOCK_EXPIRY_SECONDS`); `loadLockState` lazy-clears stale requests via `UPDATE` on read so the table self-heals without a cron. Client toasts on poll-detected resolution: holder sees a low-key "Takeover request withdrawn" info toast when pending disappears without their click (covers requester-cancel + auto-expiry); requester sees a unified "Your takeover request is no longer pending — try again if you still need to score" (denial + expiry have identical actionable outcomes from their side).
